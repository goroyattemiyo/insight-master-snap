...
## D-015: 履歴蓄積→傾向分析アーキテクチャ方針
- 日付: 2026-04-08
- 決定: 単体分析→履歴蓄積→傾向分析の段階的価値向上設計を採用
- 理由: API不要・スクショ完結の強みを維持しながら、使い続けるほど価値が上がるループを作る
- 詳細:
  - 5件未満: 単体分析のみ・プログレス表示
  - 5件以上: 傾向分析レポート解放（同SNS・直近20件でGemini横断分析）
  - 7日分: 週間レポート解放
  - 将来: InsightMaster（本家・API連携版）への導線

## D-016: 画面種別自動判定・unsupported対応
- 日付: 2026-04-08
- 決定: Geminiプロンプトに画面種別判定を追加。unsupportedのとき分析せずガイドを返す
- 理由: 「どの画面を撮ればいいかわからない」問題の根本解決
- 変更ファイル: api/gemini.js, web/result.html
- スコア: 95/100

## D-017: SNS選択画面に必要数値チップ表示
- 日付: 2026-04-08
- 決定: SNS選択後に「この数値が載っている画面を撮ってください」チップを表示
- 理由: ユーザーが正しい画面を判断できる事前ガイドとして機能する
- 変更ファイル: web/index.html

## D-018: 履歴保存データ拡張
- 日付: 2026-04-08
- 決定: 履歴にmetrics全体・screenType・strengths・weaknesses・actionsを追加保存
- 理由: 将来の傾向分析APIで蓄積データを活用するための準備
- 変更ファイル: web/result.html
